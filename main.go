...
func Auth(w http.ResponseWriter, r *http.Request) {
	...
	addOneHour := normalTime.Add(1 * time.Hour)
	user.TokenExpired = addOneHour.Unix()

	up, err := collection.UpdateOne(context.TODO(), bson.M{"token": token}, bson.D{{Key: "$set", Value: user}})
	if err != nil {
	...
	responseUser.Expires = normalTime
